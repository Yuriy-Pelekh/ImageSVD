...
namespace SVD
{
    public sealed class SingularValueDecomposition
    {
        ...
        private void Run(double[,] value, bool computeLeftSingularVectors, bool computeRightSingularVectors)
        {
            ...
            int nct;
            int nrt;
            ReduceA(aNormalized, e, work, wantU, wantV, out nct, out nrt);
            ...
            if (nct < _nCol)
                S[nct] = aNormalized[nct, nct];

            if (_mRow < p)
                S[p - 1] = 0.0;

            if (nrt + 1 < p)
                e[nrt] = aNormalized[nrt, p - 1];
        ...
        private void ReduceA(double[,] a, double[] e, double[] work, bool wantu, bool wantv, out int nct, out int nrt)
        {
        ...
        private void GenerateV(double[] e, int nrt)
        {
        ...
        private void GenerateS(double[] e, bool wantu, bool wantv, int p)
        {
            // Main iteration loop for the singular values.
            var pp = p - 1;
            var iter = 0;
        ...
        public static double[,] Denormalize(double[,] matrix)
        {
            for (var i = 0; i < matrix.GetLength(0); i++)
            {
                for (var j = 0; j < matrix.GetLength(1); j++)
